<template>
    <Layout class="favorite" :has_menu="false" :has_footer="false" title="我的订单">
        <div class="menu" slot="bar_menu" >
            <mu-menu-item title="清空" @click="clearFavorite" />
        </div>
        <div class="page_wrap">
            <div class="page_bd">
                <div class="list">
                    <div class="item" v-for="movie in list" :key="movie.id"
                            ...
                            @touchmove='touchMove'
                            @touchend='touchEnd'>
                        <div class="img" :style="`background-image: url(${movie.img})`"></div>
                        <div class="box">
                            <div class="tag" :class="`tag${movie.tag}`">{{ movie.tag | getType }}</div>
                            <div class="title">{{ movie.mo_title }}</div>
                        </div>
                    </div>
                </div>
    ...
    methods: {
        ...
        confirm() {
            ...
            this.list = this.list.filter((item) => {
             ...
             var sql=  v_order.delete.replace('?',this.tuch_item.id )
            this.$hp.post("action", {sql:sql}).then(res => {
                // this.list=res.data
            })
